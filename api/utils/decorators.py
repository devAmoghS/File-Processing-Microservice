...
def validate_user(f):
    '''
    This decorator ensures that the user logged in is the same user we are operating on
    '''
    @wraps(f)
    def func(*args, **kwargs):
        user_id = kwargs.get('user_id')
        if user_id != g.user['id']:
            abort(404, message="You do not have the permission to the resource you are trying to access")
        return f(*args, **kwargs)
    return func
...
def belongs_to_user(f):
    ...
    def func(*args, **kwargs):
        ...
        if not file or file['creator'] != user_id:
            abort(404, message="The file you are trying to access is not found")
        g.file = file
        return f(*args, **kwargs)
    return func
